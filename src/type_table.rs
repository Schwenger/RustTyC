...
use crate::{subtys::SubTys, types::UpperBoundedType, ContextType, Key};
...
#[derive(Debug, Clone)]
pub enum ResolvedSubTys<T: Clone + Debug> {
...
impl<T: Clone + Debug> FromIterator<T> for ResolvedSubTys<T> {
...
impl<T: Clone + Debug> FromIterator<(String, T)> for ResolvedSubTys<T> {
...
impl<T: Clone + Debug> ResolvedSubTys<T> {
    ...
    pub fn into_field_map(self) -> HashMap<String, T> {
        match self {
...
pub trait Constructable: ContextType {
    ...
    fn construct(&self, children: ResolvedSubTys<Self::Type>) -> Result<Self::Type, <Self as ContextType>::Err>;
}
